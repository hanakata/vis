...
<ul class="nav nav-tabs nav-justified">
	<li role="presentation" class="active"><a href="pc_info_important.php">Important</a></li>
...
<?php
session_start();
$back = isset($_SESSION['back'])?$_SESSION['back']:null;
if($back != 1){
  $pc_info = explode(":",$_POST['pc_info']);
  $_SESSION['company_name'] = $pc_info[0];
  $_SESSION['pc_name'] = $pc_info[1];
}
...
while ($row = mysqli_fetch_row($query)) {
   ...
   foreach($cve_list as $cve){
	   $query = mysqli_query($link,"SELECT * from cve_list inner join kb_list on cve_list.cve_number = kb_list.cve_number where cve_list.cve_number = '{$cve}' and severity = 'Important'");
